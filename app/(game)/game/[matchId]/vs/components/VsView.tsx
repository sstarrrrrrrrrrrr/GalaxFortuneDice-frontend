...
// 渲染 VS 过场页主视图，按模式展示玩家登场、弹幕和快捷消息。
export function VsView({
  ...
  modeName: string
  layout: 'duel' | 'three' | 'four' | 'team'
  phase: 'enter' | 'ready'
  players: Player[]
  barrages: Barrage[]
  onSendBarrage: (message: string) => void
}) {
  return (
    <main className="relative h-screen min-h-[560px] w-screen min-w-[960px] overflow-hidden bg-[#03052d] text-white">
      ...
      <section className="absolute inset-x-0 top-[16.2%] z-20 flex h-[62vh] items-center justify-center">
        {layout === 'three' && <ThreeLayout players={players.slice(0, 3)} phase={phase} />}
        {layout === 'four' && <FourLayout players={players.slice(0, 4)} phase={phase} />}
        {layout === 'team' && <TeamLayout players={players.slice(0, 4)} phase={phase} />}
        {layout === 'duel' && <DuelLayout players={players.slice(0, 2)} phase={phase} />}
      </section>

      <QuickMessages messages={quickMessages} onSend={onSendBarrage} />
